...
func Test_asposeocrcloud_DewarpImageApiService(t *testing.T) {
	...
	t.Run("Test DewarpImageApiService", func(t *testing.T) {

		// t.Skip("skip test")  // remove to run test

		filePath := "../samples/dewarping.png" // Path to your file
		...
		requestBody := *asposeocrcloud.NewOCRDewarpImageBody(fileb64Encoded)
		taskId, httpRes, err := apiClient.DewarpImageApi.PostDewarpImage(context.Background()).OCRDewarpImageBody(requestBody).Execute()
		...
		time.Sleep(10 * time.Second) // delay due to dewarping is slow process itself
		ocrResp, httpRes, err := apiClient.DewarpImageApi.GetDewarpImage(context.Background()).Id(taskId).Execute()

...
Testing UpscaleImageApiService
...
func Test_asposeocrcloud_UpscaleImageApiService(t *testing.T) {
	...
	t.Run("Test UpscaleImageApiService", func(t *testing.T) {
		...
		requestBody := *asposeocrcloud.NewOCRUpscaleImageBody(fileb64Encoded)
		taskId, httpRes, err := apiClient.UpscaleImageApi.PostUpscaleImage(context.Background()).OCRUpscaleImageBody(requestBody).Execute()
		...
		ocrResp, httpRes, err := apiClient.UpscaleImageApi.GetUpscaleImage(context.Background()).Id(taskId).Execute()

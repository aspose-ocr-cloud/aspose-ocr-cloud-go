...
Testing RecognizeReceiptApiService
...
func Test_asposeocrcloud_RecognizeReceiptApiService(t *testing.T) {
	...
	t.Run("Test RecognizeReceiptApiService", func(t *testing.T) {

		// t.Skip("skip test")  // remove to run test

		filePath := "../samples/receipt.PNG" // Path to your file
		...
		recognitionSettings := *asposeocrcloud.NewOCRSettingsRecognizeReceipt()
		recognitionSettings.Language = asposeocrcloud.LANGUAGE_ENGLISH.Ptr()
		recognitionSettings.DsrMode = asposeocrcloud.DSRMODE_DSR_NO_FILTER.Ptr()
		recognitionSettings.DsrConfidence = asposeocrcloud.DSRCONFIDENCE_DEFAULT.Ptr()
		*recognitionSettings.MakeBinarization = true
		*recognitionSettings.MakeSkewCorrect = true
		...
		*recognitionSettings.MakeSpellCheck = false
		...
		requestBody := *asposeocrcloud.NewOCRRecognizeReceiptBody(
			fileb64Encoded,
			recognitionSettings,
		)
		taskId, httpRes, err := apiClient.RecognizeReceiptApi.PostRecognizeReceipt(context.Background()).OCRRecognizeReceiptBody(requestBody).Execute()
		...
		ocrResp, httpRes, err := apiClient.RecognizeReceiptApi.GetRecognizeReceipt(context.Background()).Id(taskId).Execute()
